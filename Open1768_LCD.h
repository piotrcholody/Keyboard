...
void lcdConfiguration(void); // konfiguracja pinow do wyswietlacza
void lcdSend (uint16_t byte);
void wait_delay(int count);
uint16_t lcdRead (void);
// jak chcemy sam kolor to writeData ! 
void lcdWriteIndex(uint16_t index); //  1 uint32.t ustawia adres rejestru
void lcdWriteData(uint16_t index); // 1 uint32.t wpisuje wartosc kolor albo wspolrzedna
uint16_t lcdReadData(void); // 
void lcdWriteReg(uint16_t LCD_Reg,uint16_t LCD_RegValue); // 2 uint32.t wola dwie funkcje (podajemy adres i wartosc) i robi dwa za jednym razem
uint16_t lcdReadReg(uint16_t LCD_Reg); // odczyta wartosc spod adresu i zmieni go nawet jak byl ustawiony taki sam
